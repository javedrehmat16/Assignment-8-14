...
document.getElementById("concatenation").onclick = function () {
    ...
    let userName = "javed rehmat"
    let banger = "!"
    let sum = (message + " " + userName + banger)
    document.getElementById("input-box").innerHTML = '<p>"Any Type of Text in this String" + " " + "javed rehmat" + "!"</p>'
...
document.getElementById("ifStatementNested").onclick = function(){
    ...
    if(userName === "javed rehmat"){
        ...
        if(password == 444){
            ...
            document.getElementById("output-box").innerHTML = `<span class='text-danger'>${password}</span> is incorrect password. <br> correct password is <span class='text-success'>444</span>`
        }
    }
    else{
        document.getElementById("output-box").innerHTML = `<span class='text-danger'>${userName} </span> is incorrect user name. <br> correct user name is <span class='text-success'> "javed rehmat"</span>`
    }
}
...
document.getElementById("login").onclick = function (){
    ...
    let passWord = +prompt("please enter password")
    if (userName === "javed rehmat" && passWord === 444 ){
        document.getElementById("form").innerHTML = `<span style='color:green; font-weight:bold;'>${userName}</span>`
        document.getElementById("output-box").innerHTML = `<span style='color:green; text-transform:capitalize;'>you are successfully logged in</span>`
    }
    else {
        document.getElementById("output-box").innerHTML = "Try using javed rehmat as username and 444 as a password"
    }
